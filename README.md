...
| [kidzdev-unity-ui-overlay](https://github.com/knabsiraphop/kidzdev-unity-ui-overlay) | Loading panels (fullscreen / progress / spinner / scrim), a Spotlight highlight-cutout system for tutorials/onboarding, and a ref-counted input blocker — imperative `Show()`/dispose handles plus `Execute*` wrappers for one-line show/await/hide. UniTask-only, no DOTween, no Addressables dependency |
...
| Package | Description |
|---|---|
| [kidzdev-unity-extensions](https://github.com/knabsiraphop/kidzdev-unity-extensions) | Handy C# / Unity extension methods — strings, collections, numerics, date & time (`DateTimeOffset`, `TimeSpan`, relative time), GameObjects, Animators, Cameras, ScrollRects, GridLayoutGroups, and RectTransforms |
